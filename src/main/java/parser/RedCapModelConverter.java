    ...
    */		
    ...
    public ArrayList<AbstractObservationModel> getBooleanObs () {
    	...
    	for (int i = 0; i < hepatitis_B.size(); i++) {
			BooleanObservationModel boolobs = new BooleanObservationModel(configuration.observationSystem_HEPATITIS_B, configuration.observationCode_HEPATITIS_B_CODE, hepatitis_B.get(i));
    ...
    public ArrayList<AbstractObservationModel> getNumericalObs() {
    	...
    	for (int i = 0; i < billirubin.size(); i++) {
			NumericalObservationModel numobs = new NumericalObservationModel(configuration.observationSystem_BILIRUBIN, configuration.observationCode_BILIRUBIN_CODE, billirubin.get(i).doubleValue(), configuration.UNIT);
